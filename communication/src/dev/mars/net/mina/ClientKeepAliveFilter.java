...
import dev.mars.net.message.KeepAliveMessage;
...
public class ClientKeepAliveFilter extends IoFilterAdapter{
...
//        	LogUtils.DT(session.getId()+" 收到服务器心跳包");
    ...
    public void sessionIdle(NextFilter nextFilter, IoSession session, IdleStatus status) throws Exception {
        ...
        }else if(status==IdleStatus.READER_IDLE&&session.getIdleCount(IdleStatus.READER_IDLE)== 1){
            //READ_IDLE_CLOSE_TIMES次Read空闲就关闭session
            session.closeOnFlush();
            System.out.println("Client : "+session.getId()+" 未收到服务器心跳包，主动关闭");
